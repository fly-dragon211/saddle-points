...
def parse_cell_for_params(seed):
	...
	print "Requested fixed parameters:"
	for rf in request_fixed: print rf
...
def act_relax():
	...
	unfixed_params = np.sum([1 for p in param_fixed if not param_fixed[p]])
	step_size = 0.1 * np.sqrt(unfixed_params)

	div = ""
	for n in range(0,50): div += "="
	print div
	print "Begin saddle point search ("+str(unfixed_params)+" variable parameters)"
	print div
	...
	for iter_index in range(0,20):
		...
		if iter_index == 0:
			# For first iteration, take a random
			...
			# Get the displacement from the local minima
			xh = (x-local_minima)
			for i in range(0, len(x)):
				if param_fixed[names[i]]:
					xh[i] = 0 # This should be true anyway
...
request_fixed = [a[len("fix_"):] for a in sys.argv[1:] if a.startswith("fix_")]
